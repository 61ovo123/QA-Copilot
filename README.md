...
模块技术前端/应用框架StreamlitAI 能力DeepSeek API（通过 OpenAI SDK 兼容接口调用）数据处理pandasExcel 导出openpyxl
...
bashgit clone https://github.com/你的用户名/你的仓库名.git
cd 你的仓库名
...
项目使用 Streamlit 的 secrets.toml 机制管理密钥，不会把密钥提交到 GitHub。
...
bashcp .streamlit/secrets.toml.example .streamlit/secrets.toml


打开 .streamlit/secrets.toml，填入你自己的 Key：


tomlDEEPSEEK_API_KEY = "sk-你的真实key"
...
在左侧 requirement.md 输入框中描述需求，例如：
...
点击 Test Cases / Pytest / Postman 三个按钮中的任意一个，等待AI生成
右侧 output.log 区域会按Tab展示生成结果，点击右上角下载按钮获取文件
左侧导航栏切换到 History 可查看本次会话内的全部生成记录
